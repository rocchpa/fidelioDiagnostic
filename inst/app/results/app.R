...
get_results <- function() {
  dir <- file.path("outputs","derived")  # same place as the diagnostic app
  ...
  if (length(missing)) {
    ...
    for (s in missing) {
      # show if a file exists (any supported extension)
      files <- list.files(dir, pattern = paste0("^", s, "\\.(parquet|feather|fst|rds|csv)$"), ignore.case = FALSE)
  ...
  # 3) Report row counts before filtering empties
...
server <- function(input, output, session) {
  ...
  # Dynamic filters built from present keys (skip scenario/time)
  output$dynamic_filters <- renderUI({
    ...
    # Only create pickers with >1 unique value
    pickers <- list()
    for (k in show_keys) {
      vals <- sort(unique(as.character(DT[[k]])))
      # skip if only one unique value or all NA/empty
  ...
  observe({
    ...
    for (k in show_keys) {
      vals <- sort(unique(as.character(DT[[k]])))
      if (length(vals) <= 1 || all(!nzchar(vals))) next  # nothing to update/show
  ...
  output$plt <- renderPlot({
    # ---- SPECIAL: BITRADE_REG_t (macro-regions, c already filtered to TOT in bundle) ----
    if (input$sym == "BITRADE_REG_t") {
      ...
      p <- ggplot(BT, aes_string(x = xcol, y = yvar)) +
        ...
        labs(x = "Year", y = ylab, title = "Bilateral trade (macro regions): ff55 vs baseline") +
    ...
    # ---- SPECIAL: OUT_COMP6_SHARE_REAL_t — Δ-share scatter (policy vs baseline) ----
    if (input$sym == "OUT_COMP6_SHARE_REAL_t") {
      DT <- filteredDT(); req(!is.null(DT), NROW(DT) > 0)
      if (!"year" %in% names(DT) && "t" %in% names(DT)) DT[, year := 2014L + as.integer(t)]
      L <- to_long(DT, c("baseline","ff55")); req(!is.null(L), NROW(L) > 0)
      yr0 <- L[, min(year, na.rm = TRUE)]; yrT <- L[, max(year, na.rm = TRUE)]
      ...
      if (isTRUE(frac_like)) SC[, `:=`(baseline = 100 * baseline, ff55 = 100 * ff55)]
      if ("n" %in% names(SC)) SC[, n := macro_first(n)]
      SC <- SC[is.finite(baseline) & is.finite(ff55)]; req(NROW(SC) > 0)
    ...
    # ---- Generic plots (for GDPr_t, TBr_t, TB_GDP_t, I_PP_SECT6_t) ----
    req(input$view != "table")
    DT <- filteredDT(); req(!is.null(DT), NROW(DT) > 0)
    ...
    if (input$view == "plot_lvl") {
      ...
      if (length(gcols) == 0) { L[, grp := "all"] } else {
        L[, grp := do.call(paste, c(.SD, list(sep = "|"))), .SDcols = gcols]
      }
      L[, grp2 := paste(grp, scenario, sep = "|")]
      p <- ggplot(L, aes_string(x = xcol, y = "value", group = "grp2", color = "scenario", linetype = "scenario")) +
        geom_line() + geom_point(size = 0.8) +
        labs(x = xcol, y = label_of(input$sym), color = "Scenario", linetype = "Scenario",
      ...
      p <- ggplot(DT, aes_string(x = xcol, y = yvar, group = "grp")) +
        ...
        labs(x = xcol, y = ylab, title = paste("ff55 vs baseline —", label_of(input$sym))) +
        theme_minimal()
