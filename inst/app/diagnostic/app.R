...
promote_base_to_derived <- function(tbl, key_cols) {
  need <- c(key_cols, "scenario", "value")
  if (!all(need %in% names(tbl))) return(NULL)
  W <- data.table::dcast(tbl, as.formula(paste(paste(key_cols, collapse = " + "), "~ scenario")),
                         value.var = "value")
  if (!all(c("baseline","ff55") %in% names(W))) return(NULL)
  W <- W[!is.na(baseline) & !is.na(ff55)]
  W[, delta := ff55 - baseline]
  W[, pct   := data.table::fifelse(abs(baseline) > .Machine$double.eps, delta / baseline, NA_real_)]
  W[]
}
...
# ---- The rest of your UI/server exactly as in your script ----
# (Keep dynamic filters, bindCache, BITRADE special plot, etc.)
# Replace only the initial results loader above and small label tweaks if needed.
# ...

# Grouped choices: user sees labels, value is the symbol
...
group_of <- function(sym) meta[symbol == sym, group][1]
...
to_long <- function(DT, scenarios = c("baseline","ff55")) {
...
shallow_copy <- function(x) {
  # Use shallow() if available, otherwise fall back to copy()
  if ("shallow" %in% getNamespaceExports("data.table")) {
    data.table::shallow(x)
  } else {
    data.table::copy(x)
  }
}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectizeInput(
        "sym", "Variable:",
        choices = choices_grouped, selected = meta$symbol[1]
      ),
...
server <- function(input, output, session) {
  ...
  output$dynamic_filters <- renderUI({
    ...
    key_cols  <- setdiff(names(DT), c("baseline","ff55","delta","pct","t","year"))
    show_keys <- intersect(c("n","n1","i","c","au","oc"), key_cols)
    if (length(show_keys) == 0) return(NULL)
    
    # We create the inputs; choices are set in the observer below
  ...
  observe({
    ...
    for (k in show_keys) {
      ...
      if (k == "n") vals <- levels(macro_first(vals))  # macro regions first
  ...
  output$plt <- renderPlot({
    ...
    if (input$sym == "BITRADE_REG_t") {
      ...
      # Use precomputed columns from gdx2R: year, delta, pct (and pct_plot from filteredDT)
      ...
      p <- ggplot(BT, aes_string(x = xcol, y = yvar, color = "c", group = "c")) +
        ...
        labs(x = "Year", y = ylab, color = "Commodity",
             title = "Bilateral trade (macro regions): ff55 vs baseline") +
    ...
    if (input$sym == "OUT_COMP6_SHARE_REAL_t") {
      ...
      # Compute change (final - initial) by (n,i,scenario)
      # Cast years wide to ensure correct subtraction order
      ...
      # Now cast scenarios wide: columns 'baseline' and 'ff55' contain Δ shares
      SC <- data.table::dcast(
        End_w[, .(n, i, scenario, delta)],
        n + i ~ scenario, value.var = "delta"
      )
      
      # If shares are fractions (0–1), convert to percentage points
      frac_like <- SC[, max(abs(c(baseline, ff55)), na.rm = TRUE)] <= 1.001
      if (isTRUE(frac_like)) {
        SC[, `:=`(baseline = 100 * baseline, ff55 = 100 * ff55)]
        ylab_pp <- "Δ share (pp, 2015→2050)"
      } else {
        ylab_pp <- "Δ share (pp or % units, 2015→2050)"
      }
      
      # Order regions (optional macro-first)
      if ("n" %in% names(SC)) SC[, n := macro_first(n)]
      
      # Clean NAs (e.g., if a sector is absent in one scenario)
      SC <- SC[is.finite(baseline) & is.finite(ff55)]
      req(NROW(SC) > 0)
      
      # Scatter: x = baseline Δ, y = ff55 Δ
      p <- ggplot(SC, aes(x = baseline, y = ff55, color = i)) +
        ...
        facet_wrap(~ n) +
        labs(
          x = paste0("Baseline ", yr0, "→", yrT),
          y = paste0("ff55 ",     yr0, "→", yrT),
    ...
    # Available keys (skip scenario/time cols)
    keys <- setdiff(names(DT), c("baseline","ff55","delta","pct","t","year"))
    ...
    if (input$view == "plot_lvl") {
      ...
      p <- ggplot(DT, aes_string(x = xcol, y = yvar, group = "grp")) +
        ...
        labs(x = xcol, y = ylab,
             title = paste("ff55 vs baseline —", label_of(input$sym))) +
        theme_minimal()
